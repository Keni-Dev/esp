...
void loop() {
  ...
  // Toggle blade with debounce
  bool bladeBtn = digitalRead(BTN_BLADE);
  if (bladeBtn == LOW && lastBladeBtn == HIGH && (currentTime - lastBladeTime > debounceDelay)) {
    bladeState = !bladeState;
    lastBladeTime = currentTime;
  }
  lastBladeBtn = bladeBtn;

  // Toggle conveyor1 with debounce
  bool conv1Btn = digitalRead(BTN_CONV1);
  if (conv1Btn == LOW && lastConv1Btn == HIGH && (currentTime - lastConv1Time > debounceDelay)) {
    conv1State = !conv1State;
    lastConv1Time = currentTime;
  }
  lastConv1Btn = conv1Btn;

  // Toggle conveyor2 with debounce
  bool conv2Btn = digitalRead(BTN_CONV2);
  if (conv2Btn == LOW && lastConv2Btn == HIGH && (currentTime - lastConv2Time > debounceDelay)) {
  ...
  digitalWrite(RED_LED, bladeState ? HIGH : LOW);      // Red = Blade ON
  digitalWrite(BLUE_LED, conv1State ? HIGH : LOW);     // Blue = Conveyor 1 ON
  digitalWrite(YELLOW_LED, conv2State ? HIGH : LOW);   // Yellow = Conveyor 2 ON
