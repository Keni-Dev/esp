...
bool lastBladeBtn = false;
bool lastConv1Btn = false;
bool lastConv2Btn = false;
bool lastSpeedBtn50 = false;
bool lastSpeedBtn75 = false;
bool lastSpeedBtn100 = false;
...
void OnDataRecv(const esp_now_recv_info_t *recv_info, const uint8_t *incomingData, int len) {
  ...
  // ---- Blade Toggle ----
  if (newData.blade && !lastBladeBtn) bladeState = !bladeState;
  lastBladeBtn = newData.blade;

  // ---- Conveyor1 Toggle ----
  if (newData.conv1 && !lastConv1Btn) conv1State = !conv1State;
  lastConv1Btn = newData.conv1;

  // ---- Conveyor2 Toggle ----
  if (newData.conv2 && !lastConv2Btn) conv2State = !conv2State;
  lastConv2Btn = newData.conv2;

  // ---- Speed Toggle ----
  if (newData.wheelSpeed == 50 && !lastSpeedBtn50) wheelSpeedState = 50;
  if (newData.wheelSpeed == 75 && !lastSpeedBtn75) wheelSpeedState = 75;
  if (newData.wheelSpeed == 100 && !lastSpeedBtn100) wheelSpeedState = 100;

  lastSpeedBtn50 = (newData.wheelSpeed == 50);
  lastSpeedBtn75 = (newData.wheelSpeed == 75);
  lastSpeedBtn100 = (newData.wheelSpeed == 100);

  // Update joystick values
  receivedData.joyX = newData.joyX;
  receivedData.joyY = newData.joyY;
}
...
// ----- Loop -----
void loop() {
  // ---- Servo Control (Joystick X) ----
  if (receivedData.joyX < 1500) {
    servoAngle -= 1; // turn left slow